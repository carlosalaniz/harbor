...
import { POLKIT_RULE_PATH, SELF_UPDATE_UNIT_FILE, TAILSCALE_OPERATOR_UNIT, TOOLS_INSTALL_UNIT, DEVICE_MOUNT_UNIT, APP_CRYPTO_UNIT_FILE } from './systemd.js';
...
export async function uninstall(opts: UninstallOptions): Promise<UninstallResult> {
  ...
  for (const unit of [TAILSCALE_OPERATOR_UNIT, SELF_UPDATE_UNIT_FILE, TOOLS_INSTALL_UNIT.replace('.service', '@.service'), DEVICE_MOUNT_UNIT.replace('.service', '@.service'), APP_CRYPTO_UNIT_FILE]) {
