...
## Blockers
The GitHub repository is private: the public `curl … | sudo bash` URL, the release downloads inside `install.sh` and the daemon's update check return 404 without a token. Everything works with a local archive; Carlos decides between making the repo public or a separate public releases repository.
