...
**Blocked until the repository is public (or a public releases repository exists):** the `curl … | sudo bash` URL, the release downloads inside `install.sh`, and the daemon's update check all return 404 without a GitHub token. Everything else in this section ran end to end.
