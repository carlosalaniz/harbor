...
Harbor local preview, version 0.13.0. This report lists what actually ran, where, with which versions,
...
| `pnpm test` | unit: YAML restrictions, manifest/Compose schemas and cross-references, catalog/hash verification (incl. presentation assets), planner and renderer (bind mounts, configuration formats), host-path rules, exposure config/URLs, schema migrations v1→v7, OpenAPI (70 paths), systemd/release files (incl. `/mnt /media` writes), device mount + bind marker (round-trip, restore keeps id, replacement fresh, legacy verifies), install candidates (eligibility, non-POSIX/read-only refusal), app homes (manifest+vault, passphrase unlock, cross-machine portability) | 116 passed |
